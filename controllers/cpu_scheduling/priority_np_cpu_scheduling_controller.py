# controllers/cpu_scheduling/priority_np_cpu_scheduling_controller.py
from models.cpu_scheduling.priority_np_cpu_scheduling_model import Job, priority_np_schedule

class PriorityNPCPUController:
    ...
    def generate_gantt(self):
        ...
        # Validate that every job has a priority
        for job in jobs:
            if 'priority' not in job.extra:
                self.view.show_error(f"Process '{job.name}' is missing a priority value.")
                return
            try:
                int(job.extra['priority'])
            except ValueError:
                self.view.show_error(f"Process '{job.name}' has an invalid priority: must be an integer.")
                return

        events = priority_np_schedule(jobs)
